...
from music_modify.prefs.prefs import Settings  # pyright: ignore[reportPrivateUsage]


def set_split_values(settings: Settings):
    settings.split_text_entered = "-"
    settings.split_values_at = "-"
    settings.split_values_display = "-"


def set_tag_values(settings: Settings):
    settings.info_tags = [
...
def test_init(temp_settings: Settings):
...
def test_set_split(temp_settings: Settings):
...
def test_set_tag(temp_settings: Settings):
...
def test_resetSplit(temp_settings: Settings):
    set_split_values(temp_settings)
...
def test_resetTags(temp_settings: Settings):
    set_tag_values(temp_settings)
