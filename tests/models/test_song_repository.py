...
def test_add_single(song_path: Path):
    repo = SongRepository()
    repo.addFile(song_path)
    # assert len(repo) == 1 and (song := repo.getSong(0)) and song is not None and song.file)
    assert len(repo) == 1
    # song = repo.getSong(0)
    assert (song := repo.getSong(0)) is not None and song.file == song_path


def test_add_multiple(song_paths: list[PathLike[str]]):
    repo = SongRepository()
...
def test_reject_duplicates(song_paths: list[PathLike[str]]):
    repo = SongRepository()
    repo.addFiles(song_paths)
...
def test_remove_single(song_paths: list[PathLike[str]]):
    repo = SongRepository()
    repo.addFiles(song_paths)
    repo.removeSongs([0])
...
def test_remove_multiple(song_paths: list[PathLike[str]]):
    repo = SongRepository()
    repo.addFiles(song_paths)
